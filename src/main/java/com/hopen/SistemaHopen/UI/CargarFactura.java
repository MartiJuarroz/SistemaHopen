...
public class CargarFactura extends javax.swing.JFrame {
    ...
    private void SigBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SigBtnActionPerformed
       ...
       /*     String sql ="SELECT * from factura WHERE obra_id = ?";
        ...
        if (!numeroFac.getText().isEmpty()){
            numeroFactura = numeroFac.getText();
        }
        if (!monto.getText().isEmpty()){ 
            totalFactura = Double.parseDouble(monto.getText());
        }
        //la fecha no se como validarla todavia
        if(fechaFac.getDate() != null){
            fecha = fechaFac.getDate();
